...
for param1 in params:    
    ...
    #    print(data_fits[1].header)
        ...
        tau_exp_b1 = 10**data_fits['POSTERIOR PDF'].data['tau_exp_mean']        
        ...
        tau_exp_68_b1 = 10**data_fits['POSTERIOR PDF'].data['tau_exp_68.00']
        ...
        for i in range(len(mass_b1)):
            ...
            integral[i]  = quad(integrand, 0, msa[i])[0]
            A_b1[i] = mass_b1[i] / integral[i]
            # A_b1[i] = 1    # noticed most of the above (integration etc) was pointless as A cancels out in gradient
           
            sfr_at_msa = A_b1[i] * (msa_b1[i]*np.heaviside(tau_b1[i]-msa_b1[i], 0) + tau_b1[i]*np.exp((tau_b1[i]-msa_b1[i])/tau_exp_b1[i])*np.heaviside(msa_b1[i]-tau_b1[i], 1))
            sfr_at_msa_plus1 = A_b1[i] * ((1.01*msa_b1[i])*np.heaviside(tau_b1[i]-(1.01*msa_b1[i]), 0) + tau_b1[i]*np.exp((tau_b1[i]-(1.01*msa_b1[i]))/tau_exp_b1[i])*np.heaviside((1.01*msa_b1[i])-tau_b1[i], 1))
        ...
        params_names = ['mass', 'msa', 'tauVeff', 'metallicity', 'nebularlogU', 'tau', 'tauexp', 'nebularxi']
        params = [mass, np.log10(msa), tauV_eff, metallicity, nebular_logU, np.log10(tau), np.log10(tau_exp), nebular_xi]
        params_b1 = [mass_b1, np.log10(msa_b1), tauV_eff_b1, metallicity_b1, nebular_logU_b1, np.log10(tau_b1), np.log10(tau_exp_b1), nebular_xi_b1]
        params_68_b1 = [mass_68_b1, np.log10(msa_68_b1), tauV_eff_68_b1, metallicity_68_b1, nebular_logU_68_b1, np.log10(tau_68_b1), np.log10(tau_exp_68_b1), nebular_xi_68_b1]
    ...
    #            min_ax = 9.3
        ...
        plt.title('Input Mass (LE) vs Output Mass ({})'.format(title1), size=size)
        ...
        plt.figure(figsize=(fsize, fsize))
        plt.title('Input SFR (LE) vs Output SFR ({})'.format(title1), size=size)
        ...
        plt.figure(figsize=(fsize, fsize))
        plt.title('Input SSFR (LE) vs Output SSFR ({})'.format(title1), size=size)
