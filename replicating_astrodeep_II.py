...
print(len(cat['ZBEST'][cat['ZBEST'] == -1]))
print(len(cat['RELFLAG'][cat['RELFLAG'] == 0]))
print(len(cat['RELFLAG'][cat['RELFLAG'] == 1]))
print(len(cat['RELFLAG']))
print(len(cat['RELFLAG'][   (cat['RELFLAG'] == 1)    &    (cat['B435'] < 0)    ]))
print(len(cat['RELFLAG'][   (cat['RELFLAG'] == 0)    &    (cat['B435'] < 0)    ]))

print(len(cat['RELFLAG'][    (cat['B435'] < 0)    ]))
print(len(cat['RELFLAG'][    (cat['B435'] == 0)    ]))
print(len(cat['RELFLAG'][    (cat['B435'] > 0)    ]))
...
for i in [0, 1, 2, 3]:
    plt.xlim(0, 10)
    plt.ylim(0, 300)
    plt.hist(cat['ZBEST'][(cat['field'] == i) & (cat['ID'] < 20000)], bins=np.arange(0, 10, 0.1))
    plt.show()
 '''  

q75, q25 = np.percentile(cat['ZBEST'], [75 ,25])
iqr = q75 - q25
print(iqr)
...
bin_num = (max(cat['ZBEST']) - min(cat['ZBEST'])) / h
