...
filter_label = ['b_B435', 'b_V606', 'b_I814', 'b_Y105', 'b_J125', 'b_JH140', 'b_H160', 'b_Ks', 'b_CH1', 'b_CH2'] #column name from ASTRODEEP config file
...
for i in range(samples):
    ...
    f_spec_fit_arr.append(data_fits['FULL SED'].data[idx][0]/(1+z_fit_arr[i]))
...
f_spec_fit_min = f_spec_fit_arr[0]
f_spec_fit_max = f_spec_fit_arr[0]
...
for i in range(samples-1):
    f_spec_fit_min = np.minimum(f_spec_fit_min, f_spec_fit_arr[i+1])
    f_spec_fit_max = np.maximum(f_spec_fit_max, f_spec_fit_arr[i+1])
...
plt.plot(wl_spec_mock, f_spec_mock*wl_spec_mock, linewidth=0.5, zorder=2, label='Input SED')
plt.fill_between(wl_spec_mock, f_spec_fit_min*wl_spec_mock, f_spec_fit_max*wl_spec_mock, alpha=0.3, color='k', linewidth=0, zorder=1, label='Output SED samples') 
