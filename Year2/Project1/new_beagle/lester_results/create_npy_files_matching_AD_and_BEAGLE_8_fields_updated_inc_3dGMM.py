...
cut = 0
cut_i = 0
#for i, field in enumerate(field_AD[3570:3580]):
for i, field in enumerate(field_AD):
    ...
    if field > cut:
        ...
        # SAVE
        # =============================================================================
        np.save('./npy_files/'+subfolder+'_id_BEAGLE_{}'.format(cut),id_BEAGLE)
        np.save('./npy_files/'+subfolder+'_mass_BEAGLE_tot_{}'.format(cut),mass_BEAGLE_tot)
        np.save('./npy_files/'+subfolder+'_mass_BEAGLE_stellar_{}'.format(cut),mass_BEAGLE_stellar)
        np.save('./npy_files/'+subfolder+'_sfr_BEAGLE_instant_{}'.format(cut),sfr_BEAGLE_instant)
        np.save('./npy_files/'+subfolder+'_redshift_BEAGLE_{}'.format(cut),redshift_BEAGLE)
        
        np.save('./npy_files/'+subfolder+'_tau_BEAGLE_{}'.format(cut),tau_BEAGLE)
        np.save('./npy_files/'+subfolder+'_tauv_BEAGLE_{}'.format(cut),tauv_BEAGLE)
        np.save('./npy_files/'+subfolder+'_msa_BEAGLE_{}'.format(cut),msa_BEAGLE)
        np.save('./npy_files/'+subfolder+'_metallicity_BEAGLE_{}'.format(cut),metallicity_BEAGLE)
        
        np.save('./npy_files/'+subfolder+'_min_chi2_BEAGLE_{}'.format(cut),min_chi2_BEAGLE)
        
        ##GMM
        np.save('./npy_files/'+subfolder+'_id_GMM_2d_{}'.format(cut),id_GMM_2d)
        np.save('./npy_files/'+subfolder+'_x_GMM_2d_{}'.format(cut),x_GMM_2d)
        np.save('./npy_files/'+subfolder+'_y_GMM_2d_{}'.format(cut),y_GMM_2d)
        np.save('./npy_files/'+subfolder+'_xsig_GMM_2d_{}'.format(cut),xsig_GMM_2d)
        np.save('./npy_files/'+subfolder+'_ysig_GMM_2d_{}'.format(cut),ysig_GMM_2d)
        np.save('./npy_files/'+subfolder+'_xycov_GMM_2d_{}'.format(cut),xycov_GMM_2d)
        np.save('./npy_files/'+subfolder+'_amp_GMM_2d_{}'.format(cut),amp_GMM_2d)
        
        np.save('./npy_files/'+subfolder+'_id_GMM_3d_{}'.format(cut),id_GMM_3d)
        np.save('./npy_files/'+subfolder+'_x_GMM_3d_{}'.format(cut),x_GMM_3d)
        np.save('./npy_files/'+subfolder+'_y_GMM_3d_{}'.format(cut),y_GMM_3d)
        np.save('./npy_files/'+subfolder+'_z_GMM_3d_{}'.format(cut),z_GMM_3d)
        np.save('./npy_files/'+subfolder+'_xsig_GMM_3d_{}'.format(cut),xsig_GMM_3d)
        np.save('./npy_files/'+subfolder+'_ysig_GMM_3d_{}'.format(cut),ysig_GMM_3d)
        np.save('./npy_files/'+subfolder+'_zsig_GMM_3d_{}'.format(cut),zsig_GMM_3d)
        np.save('./npy_files/'+subfolder+'_xycov_GMM_3d_{}'.format(cut),xycov_GMM_3d)
        np.save('./npy_files/'+subfolder+'_xzcov_GMM_3d_{}'.format(cut),xzcov_GMM_3d)
        np.save('./npy_files/'+subfolder+'_yzcov_GMM_3d_{}'.format(cut),yzcov_GMM_3d)
        np.save('./npy_files/'+subfolder+'_amp_GMM_3d_{}'.format(cut),amp_GMM_3d)
    ...
    fileName = '/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/data/astrodeep_field_{}_subset_RF1_001.fits'.format(int(field))
...
#    GET BEAGLE ID FROM ORIGINAL
    ...
    if len(np.where(id_original==id_AD[i])[0]) == 0:
        ...
        fileName = '/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/lester_results/{}/summary_catalogues/{}_BEAGLE_summary_catalogue.fits'.format(subfolder, int(field))
        data_fits = fits.open(fileName)
    #    print(data_fits.info())
    ...
    #    GET BEAGLE MASS, SFR and Z
        ...
        if len(np.where(id_input==id_BEAGLE[i-cut_i])[0]) == 0:
            ...
            # INSTANT SFR
            if subfolder == 'danger_constant':
                sfr_BEAGLE_instant.append(np.log10(data_fits['STAR FORMATION'].data['SFR_median'][idx]))
            else:
                data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/lester_results/{}/instant_sfr_medians/{}_instant_sfr_medians.p'.format(subfolder, int(field)),'r'))
            #    print(data.keys())
            ...
            # CHI2
            fileName = '/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/lester_results/{}/chi2/{}_chi2.fits'.format(subfolder, int(field))
            data_fits = fits.open(fileName)
            #print(data_fits.info())
            #print(data_fits[1].header)
            ...
            # GMM 2d
            data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/lester_results/{}/kelly_2d_GMM_inputs/{}{}_GMM_2d_fits.p'.format(subfolder, int(field), mass_sfr_option),'r'))
        #    print(data.keys())
            idx_sort = np.argsort(np.asarray(data['id'], float))
            ...
            # GMM 3d
            data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/new_beagle/lester_results/{}/kelly_3d_GMM_inputs/{}{}_GMM_3d_fits.p'.format(subfolder, int(field), mass_sfr_option),'r'))
        #    print(data.keys())
            idx_sort = np.argsort(np.asarray(data['id'], float))
