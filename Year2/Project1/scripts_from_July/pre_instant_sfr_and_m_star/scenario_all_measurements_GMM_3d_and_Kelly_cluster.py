...
def run_Kelly07(inputDict):
    ...
    while success == False and counter < 5:
        counter = counter+1
        command =   "python -W ignore run_GMM_3d_and_Kelly_sampler_cluster.py -f "+inputDict['folder']+\
...
#perform GMM fits
if args.runGMMfits:
    command = "python -W ignore run_GMM_3d_and_Kelly_sampler_cluster.py -f "+args.resultsFolder+" -g --n-gauss 3 --no-limits"
    if args.mTot:
        command = command + " --m-tot"
    print(command)
    os.system(command)
