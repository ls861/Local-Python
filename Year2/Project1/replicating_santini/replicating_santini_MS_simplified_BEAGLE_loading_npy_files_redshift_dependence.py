...
zLow = 1.3
zHigh = 2.0
san_bin = 0 # 1.65, 2.5, 3.5, 4.5, 5.5
wLim = (zHigh - zLow) / 2.0
zLim = zLow + wLim
...
if plot_input_to_2sigma_heatplot == 1:
    ...
    plt.hexbin(heatplot_mass, heatplot_sfr, gridsize=[100,100])
    plt.xlim(7, 10)
    plt.ylim(-2, 3)
    plt.tight_layout()
    plt.title('${} < z < {}$'.format(zLow, zHigh))
    ...
    plt.hist2d(heatplot_mass, heatplot_sfr, bins=100)
    plt.xlim(7, 10)
    plt.ylim(-2, 3)
    plt.tight_layout()
    plt.title('${} < z < {}$'.format(zLow, zHigh))
