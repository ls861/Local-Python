...
#        print(sfr)
        ...
        appMag_1600K = (-2.5*(beta+2.0)*np.log10(1600*(1+redshift_med[i]))) + fit[1] # no A_1600 correction yet
        F_vK = 10**((appMag_1600K - 8.90)/-2.5) # Jy
        F_vK = F_vK * 1e-26 # W Hz-1 m-2
        L_vK = F_vK * 4 * np.pi * (D_l**2) / (1+redshift_med[i]) # W Hz-1
        vL_vK = (299792458 / (1600*1e-10)) * L_vK # W
        vL_vK = vL_vK * 1e7 # erg s-1
        
        tauK = A_1600/1.086
        vL_vK = vL_vK * np.exp(tauK)
        
        sfrK = np.log10(vL_vK) - 43.35 # log (solar masses per year)
...
plt.scatter(sfrArr, sfrArrK)
plt.xlim
plt.show()
    
plt.hist(np.array(sfrArr)-np.array(sfrArrK))
