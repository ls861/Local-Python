...
#        plt.show()
    ...
    # =============================================================================
    # replicating santini figure 1
    # =============================================================================
        
    with np.errstate(divide='ignore', invalid='ignore'):
        test = {    'field_AD':             AD['field'],
                    'id_AD':                AD['ID'], 
                    'mag_AD':               np.log10(AD['MAGNIF']), 
                    'redshift_AD':          AD['ZBEST'], 
                    'mass_AD':              np.log10(AD['MSTAR']*1e9) - np.log10(AD['MAGNIF']), 
    ...
    # =============================================================================
    # H band mag cut of < 27.5 USING FLUXES
    # =============================================================================
    
    ABmag = -2.5*np.log10(AD['b_H160']*1e-6) + 8.90
#    plt.scatter(ABmag, AD['H160'], alpha=0.01)
#    plt.xlabel('ABmag from flux')
#    plt.ylabel('ABmag from AD')
#    plt.show()
    ...
    binwidth = 0.25
    # 1 is clusters, 2 is H<27.5, 3 is redshift, 4 is lower mass, 5 is relflag, 6 is upper mass
    idx = np.logical_and(idx1,idx2)
#    idx = np.logical_and(idx1,idx00)
    idx = np.logical_and(idx,idx3)
    idx = np.logical_and(idx,idx5)
    plt.title('${} < z < {}$'.format(zLow, zHigh))
    plt.hist(test['mass_AD_neb'][idx], bins=np.arange(6.5, 20, binwidth), color='lightgray')   
    plt.hist(test['mass_AD_neb'][idx]+test['mag_AD'][idx], bins=np.arange(6.5, 20, binwidth), histtype=u'step', edgecolor='k')
