...
np.seterr(all="raise")
os.system("mkdir "+args.folder+"sklearn_GMM_3d_"+fileStr+"/")

# GMM fits
if args.reRun:
    os.system('rm '+args.folder+"sklearn_GMM_3d_"+fileStr+"/*GMM_3d.p")
...
for file in fileList:
    if '.fits.gz' in file:
        ...
        fileTest = os.path.isfile(args.folder+"sklearn_GMM_3d_"+fileStr+"/"+pickleName)
        if args.reRun or fileTest == False:
            ...
            tryCounter = 0
            fitted = False
            while fitted == False and tryCounter <= 500:
            ...
            if fitted == True:
                ...
                pickle.dump({'x':gmm.means_[:,0], 'y':gmm.means_[:,1], 'z':gmm.means_[:,2], 'xsig':temp_xsig, 'ysig':temp_ysig, 'zsig':temp_zsig, 'xycov':temp_xycov, 'xzcov':temp_xzcov, 'yzcov':temp_yzcov, 'amp':gmm.weights_}, open(args.folder+"sklearn_GMM_3d_"+fileStr+"/"+pickleName,'w'))
...
os.system('mkdir ./kelly_3d_GMM_inputs/')
sbf = './kelly_3d_GMM_inputs/{}_'.format(fold)

if args.reRun:
    os.system('rm '+sbf+'GMM_3d_fits.p')

np.seterr(under="warn")
fileTest = os.path.isfile(sbf+'GMM_3d_fits.p')
...
fileList = os.listdir(args.folder+"sklearn_GMM_3d_"+fileStr)

for file in fileList:
    print(file)
    if '_3d.p' in file and '.pdf' not in file:
        data = pickle.load(open(args.folder+"sklearn_GMM_3d_"+fileStr+"/"+file,"r"))
...
z = np.array(z)[idx_sort]
xycov = np.array(xycov)[idx_sort]
xzcov = np.array(xzcov)[idx_sort]
yzcov = np.array(yzcov)[idx_sort]
xsig = np.array(xsig)[idx_sort]
ysig = np.array(ysig)[idx_sort]
zsig = np.array(zsig)[idx_sort]
amp = np.array(amp)[idx_sort]

pickle.dump({'id':idArr, 'x':x, 'y':y, 'z':z, 'xycov':xycov, 'xzcov':xzcov, 'yzcov':yzcov, 'xsig':xsig, 'ysig':ysig, 'zsig':zsig, 'amp':amp, 'nGauss':args.nGauss},open(sbf+'GMM_3d_fits.p','w'))
print('dumped: ', args.folder)

# os.system('rm -r '+args.folder+"sklearn_GMM_3d_"+fileStr+"/")
