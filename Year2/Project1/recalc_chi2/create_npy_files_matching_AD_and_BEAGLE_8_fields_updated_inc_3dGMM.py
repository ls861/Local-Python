...
#    GET BEAGLE ID FROM ORIGINAL
    ...
    #    GET BEAGLE MASS, SFR and Z
        ...
        if len(np.where(id_input==id_BEAGLE[i-cut_i])[0]) == 0:
            ...
            # NEW CHI2
            fileName = '/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/from_cluster_chi2/{}_new_chi2.fits'.format(int(field))
            data_fits = fits.open(fileName)
            #print(data_fits.info())
            #print(data_fits[1].header)
            #id_chi2 = np.asarray(data_fits[1].data['id'], dtype=int)
            new_chi2 = data_fits[1].data['new_chi2']
            data_fits.close()
        ...
        #    print(data.keys())
            ...
            # GMM 3d
            data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/GMM/kelly_3d_GMM_inputs/{}_mStar_delayed_GMM_3d_fits.p'.format(int(field)),'r'))
        #    print(data.keys())
            idx_sort = np.argsort(np.asarray(data['id'], float))
