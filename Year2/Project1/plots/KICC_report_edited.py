...
#    rc_mock_z1 = read_chain(1.25, 2.0, chain_MS_mock_z1, 'const_beta_linear_alpha')
    ...
    rc_c1p = read_chain(1.25, 2.0, chain_MS_29_c1p, 'const_beta_ssfr_alpha')
    rc_c2p = read_chain(2.0, 3.0, chain_MS_29_c2p, 'const_beta_ssfr_alpha')
    rc_c3p = read_chain(3.0, 4.0, chain_MS_29_c3p, 'const_beta_ssfr_alpha')
    rc_c4p = read_chain(4.0, 5.0, chain_MS_29_c4p, 'const_beta_ssfr_alpha')
    rc_c5p = read_chain(5.0, 6.0, chain_MS_29_c5p, 'const_beta_ssfr_alpha')
    rc_cp = read_chain(1.25, 6.0, chain_MS_29_cp, 'const_beta_ssfr_alpha')
    rc_c1kp = read_chain(1.25, 2.0, chain_MS_29_c1kp, 'const_beta_ssfr_alpha')
...
for i in range(len(s['mass_BEAGLE_stellar'])):
...
x_hp = x_hp[abs(z_hp - ((z_upper+z_lower)/2.0)) < ((z_upper-z_lower)/2.0)]
y_hp = y_hp[abs(z_hp - ((z_upper+z_lower)/2.0)) < ((z_upper-z_lower)/2.0)]
z_hp = z_hp[abs(z_hp - ((z_upper+z_lower)/2.0)) < ((z_upper-z_lower)/2.0)]
...
ax1.plot(x_tmp, (x_tmp-9.7)*rc['beta_50_arr'][0] + rc['alpha_50_arr'][0], color='r', linewidth=lw, label=r'Our Work, {} $<$ z $<$ {}'.format(str(z_lower), str(z_upper)))
#ax1.plot(x_tmp, (x_tmp-9.7)*rc_c1['beta_50_arr'][0] + rc_c1['alpha_50_arr'][0], color='w')

xi_min = 8.5
xi_max = 10.0    
sig = rc['sig0_50_arr'][0] * ( ((1.0-rc['k_50_arr'][0])*(x_tmp-xi_max)/(xi_max-xi_min)) + 1.0 )
ax1.plot(x_tmp, (x_tmp-9.7)*rc['beta_50_arr'][0] + rc['alpha_50_arr'][0] + sig, color='r', linestyle='dashed', linewidth=lw, label=r'Our Work, Intrinsic Scatter')
ax1.plot(x_tmp, (x_tmp-9.7)*rc['beta_50_arr'][0] + rc['alpha_50_arr'][0] - sig, color='r', linestyle='dashed', linewidth=lw)
...
if save:
    plt.savefig('KICC_10.png', dpi=300, transparent=False, bbox_inches='tight')
plt.show()
...
z_lower = 5.0
z_upper = 6.0
medians = get_medians(chain_MS_29_c5)
...
log_p_xi_eta_theta = norm.logpdf(s['sfr_BEAGLE_instant'], scale=medians['sig0'], loc=sfr_surface_real)
log_p_eta_xi_theta = norm.logpdf(s['sfr_BEAGLE_instant'], scale=medians['sig0'], loc=sfr_surface_real)
p_bad = norm.pdf(s['sfr_BEAGLE_instant'], scale=medians['outlier_sigma'], loc=medians['outlier_mean'])
...
scatter = ax1.scatter(s['mass_BEAGLE_stellar'][idx_sort], s['sfr_BEAGLE_instant'][idx_sort], c=np.log10((z_good/z_bad)[idx_sort]), cmap=mpl.cm.get_cmap('coolwarm'), alpha=1.0, vmin=-2.5, vmax=2.5, s=100)

print(len(s['mass_BEAGLE_stellar']))
...
if save:
    plt.savefig('KICC_11.png', dpi=300, transparent=False, bbox_inches='tight')
plt.show()
...
for rc in [rc_c1, rc_c1_lm8p5, rc_c1_lm9p0, rc_c2, rc_c3, rc_c4, rc_c5]:
...
for a, ax in enumerate(axes):
    ax.plot(0, 0, color='#1f77b4', label='z1 lm8p0')
    ax.plot(0, 0, color='#ff7f0e', label='z1 lm8p5')
    ax.plot(0, 0, color='#2ca02c', label='z1 lm9p0')
    ax.plot(0, 0, color='#d62728', label='z2')
    ax.plot(0, 0, color='#9467bd', label='z3')
    ax.plot(0, 0, color='#8c564b', label='z4')
    ax.plot(0, 0, color='#e377c2', label='z5')
...
if save:
    plt.savefig('015_params_vs_redshift.png', dpi=300, transparent=False, bbox_inches='tight')
plt.show()
