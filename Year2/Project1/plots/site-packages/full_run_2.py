...
def full_run_2(chain_MS_29_c1, chain_MS_29_c2, chain_MS_29_c3, chain_MS_29_c4, chain_MS_29_c5, chain_MS_29_c1k, s29z1, s29z2, s29z3, s29z4, s29z5, rc_c1, rc_c2, rc_c3, rc_c4, rc_c5, rc_c1k, rc_c, normalisation, figuresize, fontsize_legend, fontsize_axes, string_slope, string_normalisation, string_scatter, string_ssfr, save):
    ...
    for i in range(len(s)):
        ...
        ax[i].plot(x_tmp, (x_tmp-9.7)*rc[i].item().get('beta_50_arr')[0] + rc[i].item().get('alpha_50_arr')[0], color='r', linewidth=lw, label=r'Our Work, {} $<$ z $<$ {}'.format(str(z_lower[i]), str(z_upper[i])))
         
        
        sig = rc[i].item().get('sig0_50_arr')[0] * ( ((1.0-rc[i].item().get('k_50_arr')[0])*(x_tmp-xi_max)/(xi_max-xi_min)) + 1.0 )
        ax[i].plot(x_tmp, (x_tmp-9.7)*rc[i].item().get('beta_50_arr')[0] + rc[i].item().get('alpha_50_arr')[0] + sig, color='r', linestyle='dashed', linewidth=lw, label=r'Our Work, Intrinsic Scatter')
        ax[i].plot(x_tmp, (x_tmp-9.7)*rc[i].item().get('beta_50_arr')[0] + rc[i].item().get('alpha_50_arr')[0] - sig, color='r', linestyle='dashed', linewidth=lw)
    ...
    ax1.plot(z_225, (np.log10((1+z_225)**2.25) - 9.0) * (rc_c.item().get('ssfr_50_arr')[idx] / (np.log10((1+2.0)**2.25) - 9.0)), color='gray', linestyle='--', label='$\sim(1+z)^{2.25}$') # normalise Dekel et al. 2009 to z=2
    ...
    x_tmp = np.array([xlow, xhigh])
    ax1.plot(x_tmp, (x_tmp-9.7)*rc_c1k.item().get('beta_50_arr')[0] + rc_c1k.item().get('alpha_50_arr')[0], color='r', linewidth=lw, label=r'Our Work, 1.25 $<$ z $<$ 2.0')
     
    sig = rc_c1k.item().get('sig0_50_arr')[0] * ( ((1.0-rc_c1k.item().get('k_50_arr')[0])*(x_tmp-xi_max)/(xi_max-xi_min)) + 1.0 )
    ax1.plot(x_tmp, (x_tmp-9.7)*rc_c1k.item().get('beta_50_arr')[0] + rc_c1k.item().get('alpha_50_arr')[0] + sig, color='r', linestyle='dashed', linewidth=lw, label=r'Our Work, Intrinsic Scatter')
    ax1.plot(x_tmp, (x_tmp-9.7)*rc_c1k.item().get('beta_50_arr')[0] + rc_c1k.item().get('alpha_50_arr')[0] - sig, color='r', linestyle='dashed', linewidth=lw)
    ...
    ax2.hist(chain_MS_29_c1k['sig0']*chain_MS_29_c1k['k'], alpha=0.3, bins=30, density=True, range=[0.1, 1.0], label=r'$\log(\mathrm{Stellar} \, \mathrm{Mass} \, / \, \mathrm{M_{\odot}}) = 8.5$')
    ax2.hist(chain_MS_29_c1k['sig0'], alpha=0.3, bins=30, density=True, range=[0.1, 1.0], label=r'$\log(\mathrm{Stellar} \, \mathrm{Mass} \, / \, \mathrm{M_{\odot}}) = 10.0$')
    ...
    ax2.legend(loc='upper right', frameon=True, fontsize=1.44*fontsize_legend)
    ...
    for rc in [rc_c1, rc_c2, rc_c3, rc_c4, rc_c5]:
        for a, ax in enumerate(axes):
            ax.plot(rc.item().get('z'), rc.item().get('{}_50_arr'.format(param[a])), color='blue')
            ax.fill_between(rc.item().get('z'), rc.item().get('{}_16_arr'.format(param[a])), rc.item().get('{}_84_arr'.format(param[a])), alpha=0.1, color='blue', zorder=0)
    ...
    # layout
    ylim_low = [0.2, 0.1, 0.01]
    ylim_high = [1.4, 2.4, 0.79]
