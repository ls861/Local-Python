...
mass_BEAGLE = d[1].data['mass_BEAGLE_stellar'] + d[1].data['mag_AD']

sort = np.argsort(mass_BEAGLE)

mass_BEAGLE = mass_BEAGLE[sort]

field_AD = d[1].data['field_AD'][sort]

id_AD = d[1].data['id_AD'][sort]
z_AD = d[1].data['redshift_AD'][sort]
mass_AD = d[1].data['mass_AD_neb'][sort]

id_BEAGLE = d[1].data['id_BEAGLE'][sort]
z_BEAGLE = d[1].data['redshift_BEAGLE'][sort]
# mass_BEAGLE = d[1].data['mass_BEAGLE_stellar']

RA = d[1].data['RA_AD'][sort]
DEC = d[1].data['DEC_AD'][sort]
...
for i in range(len(id_AD)):
    ...
    # if uid in ['2_1192', '4_1301', '2_541', '6_2907', '6_2371']: # ORDER OF BEAGLE MASS TO MATCH BLUE POINTS IN FIG 13
        print(uid, mass_BEAGLE[i])
...
#                axarr[fi].scatter(s_zoom/2., s_zoom/2., marker='o', color='r', s=500, facecolors='none')
            ...
            axarr[count, fi].get_xaxis().set_ticks([])
            axarr[count, fi].get_yaxis().set_ticks([])
            # axarr[count, fi].xaxis.set_tick_params(which='minor', size=0, width=0)
            # axarr[count, fi].xaxis.set_tick_params(labelsize=20)
            # axarr[count, fi].yaxis.set_tick_params(which='both', left=False, right=False)
            # axarr[count, fi].yaxis.set_tick_params(which='minor', size=0, width=0)
            # axarr[count, fi].yaxis.set_tick_params(labelsize=2)
