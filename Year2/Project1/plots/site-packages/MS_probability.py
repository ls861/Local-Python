...
def MS_probability(ADx_subset, medians, normalisation, figuresize, fontsize_axes, string_mass, string_sfr, string_prob_ratio, save):
    ...
    ax1.plot((normalisation, normalisation), (-10, 10), color='k', alpha=1.0, linestyle='dashed', lw=3)
    ...
    ax1.scatter(ADx_subset['mass_BEAGLE_stellar'][idx_sort][ADx_subset['redshift_BEAGLE'][idx_sort]>4], ADx_subset['sfr_BEAGLE_instant'][idx_sort][ADx_subset['redshift_BEAGLE'][idx_sort]>4], c=5+np.log10((z_good/z_bad)[idx_sort][ADx_subset['redshift_BEAGLE'][idx_sort]>4]), cmap=mpl.cm.get_cmap('coolwarm'), alpha=1.0, vmin=-2.5, vmax=2.5)
    ...
    if save:
        plt.savefig('MS_probability.png', dpi=300, transparent=False, bbox_inches='tight')
    plt.show()
