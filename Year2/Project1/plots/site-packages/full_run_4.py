...
def full_run_4(chain_MS_29_c1, chain_MS_29_c2, chain_MS_29_c3, chain_MS_29_c4, chain_MS_29_c5, s29z1, s29z2, s29z3, s29z4, s29z5, rc_c1, rc_c2, rc_c3, rc_c4, rc_c5, rc_c, normalisation, figuresize, fontsize_legend, fontsize_axes, string_slope, string_normalisation, string_scatter, string_ssfr, string_redshift, save):
    ...
    mpl.rcParams.update({'errorbar.capsize': 0})
    ...
    # Stark+13
    # =============================================================================
    z_star = np.array([3.7999, 4.9774, 5.8916, 5.8916, 6.7911, 6.7911])
    ssfr_star = np.array([0.7522, 0.7332, 0.8332, 0.7808, 1.1191, 0.9524]) - 9.0
    ssfr_p_star = np.array([1.0715, 1.0476, 1.1048, 1.1048, 1.2859, 1.2859]) - ssfr_star - 9.0
    ssfr_m_star = ssfr_star + 9.0 - np.array([0.4663, 0.4378, 0.4997, 0.4997, 0.676, 0.676])
    ssfr_star += s2c_SFR - s2c_M
    ...
    # INTRINSIC # We find that the average intrinsic scatter in SFR across the mass bins to be σ = 0.26±0.04, 0.23±0.10, and 0.34±0.11 at z ∼ 4, 5, and 6, respectively.
    sig_salm = np.array([0.26, 0.23, 0.34])
    sig_err_salm = np.array([0.04, 0.10, 0.11])
    ...
    for rc in [rc_c1, rc_c2, rc_c3, rc_c4, rc_c5]:
        for a, ax in enumerate(axes):
            ax.plot(rc.item().get('z'), rc.item().get('{}_50_arr'.format(param[a])), color='k', zorder=10, linewidth=lw)
            ax.fill_between(rc.item().get('z'), rc.item().get('{}_16_arr'.format(param[a])), rc.item().get('{}_84_arr'.format(param[a])), alpha=0.1, color='blue', edgecolor='w', zorder=10)

    for rc in [rc_c]:
        for a, ax in enumerate(axes):
            ax.plot(rc.item().get('z'), rc.item().get('{}_50_arr'.format(param[a])), color='red', zorder=10, linewidth=lw)
            ax.fill_between(rc.item().get('z'), rc.item().get('{}_16_arr'.format(param[a])), rc.item().get('{}_84_arr'.format(param[a])), alpha=0.3, color='red', edgecolor='w', zorder=10)


    # layout
    ylim_low = [0.1, 0.2, -0.05]
    ylim_high = [2.4, 1.4, 0.65]
    ...
    # ax1.errorbar(z_stei, alphaN_stei, yerr=alphaN_err_stei, ls='none', marker='.', ms=12, label='Steinhardt+14')
    # ax1.errorbar(z_whit, alphaN_whit, yerr=alphaN_err_whit, ls='none', marker='.', ms=12, label='Whitaker+14')
    # ax1.errorbar(z_salm, alphaN_salm1, yerr=alphaN_err_salm1, ls='none', marker='.', ms=12, label='Salmon+15')
    # ax1.errorbar(z_kurc, alphaN_kurc, yerr=alphaN_err_kurc, ls='none', marker='.', ms=12, label='Kurczynski+16')
    # ax1.errorbar(z_sant, alphaN_sant, yerr=alphaN_err_sant, ls='none', marker='.', ms=12, label='Santini+17')
    # ax1.errorbar(z_boog, alphaN_boog, yerr=alphaN_err_boog, ls='none', marker='.', ms=12, label='Boogaard+18')
    # ax1.errorbar(z_pear, alphaN_pear, yerr=alphaN_err_pear, ls='none', marker='.', ms=12, label='Pearson+18')
    # ax1.errorbar(z_donn, alphaN_donn, yerr=alphaN_err_donn, ls='none', marker='.', ms=12, label='Donnari+19')

    # ax1.plot(z_spea, alphaN_spea, label='Speagle+14')
    # ax1.plot(z_schr, alphaN_schr, label='Schreiber+15')
    # ax1.plot(z_leja, alphaN_leja, label='Leja+21')

    # ax1.fill_between(z_spea, alphaN_spea-alphaN_err_spea, alphaN_spea+alphaN_err_spea, alpha=0.1, color='k')
    # ax1.fill_between(z_schr, alphaN_schr-alphaN_err_schr, alphaN_schr+alphaN_err_schr, alpha=0.1, color='k')


    ax1.errorbar(z_donn, alphaN_donn, ls='dashed', marker='.', ms=0, label='Illustris-TNG')
    ax1.errorbar(z_love, alphaN_love, ls='dashed', marker='.', ms=0, label='FLARES')


    ax1.errorbar(z_spea, alphaN_spea, ls='solid', marker='.', ms=0, label='Speagle+14')
    ax1.plot(3, 1.0)
    ax1.errorbar(z_schr, alphaN_schr, ls='solid', marker='.', ms=0, label='Schreiber+15')
    ax1.errorbar(z_leja, alphaN_leja, ls='solid', marker='.', ms=0, label='Leja+21')

    ax1.errorbar(z_stei, alphaN_stei, yerr=alphaN_err_stei, ls='none', marker='.', ms=ms_dot, label='Steinhardt+14')
    ax1.errorbar(z_whit, alphaN_whit, yerr=alphaN_err_whit, ls='none', marker='.', ms=ms_dot, label='Whitaker+14')
    ax1.errorbar(z_salm, alphaN_salm1, yerr=alphaN_err_salm1, ls='none', marker='.', ms=ms_dot, label='Salmon+15')
    ax1.errorbar(z_kurc, alphaN_kurc, yerr=alphaN_err_kurc, ls='none', marker='.', ms=ms_dot, label='Kurczynski+16')
    ax1.errorbar(z_sant, alphaN_sant, yerr=alphaN_err_sant, ls='none', marker='.', ms=ms_dot, label='Santini+17')
    # ax1.errorbar(z_boog, alphaN_boog, yerr=alphaN_err_boog, ls='none', marker='.', ms=ms_dot, label='Boogaard+18')
    ax1.errorbar(z_pear, alphaN_pear, yerr=alphaN_err_pear, ls='none', marker='.', ms=ms_dot, label='Pearson+18')

    ax1.fill_between(z_schr, alphaN_schr-alphaN_err_schr, alphaN_schr+alphaN_err_schr, alpha=0.1, color='#d62728')

    ax1.plot(0, 0, color='red', label='Our Work', linewidth=lw, zorder=0)
    ax1.plot(0, 0, color='k', label='Redshift Bins', linewidth=lw, zorder=0)

    # ax2.errorbar(z_stei, beta_stei, yerr=beta_err_stei, ls='none', marker='.', ms=12, label='Steinhardt+14')
    # ax2.errorbar(z_whit, beta_whit, yerr=beta_err_whit, ls='none', marker='.', ms=12, label='Whitaker+14')
    # ax2.errorbar(z_salm, beta_salm1, ls='none', marker='x', ms=8, label='Salmon+15')
    # ax2.errorbar(z_kurc, beta_kurc, yerr=beta_err_kurc, ls='none', marker='.', ms=12, label='Kurczynski+16')
    # ax2.errorbar(z_sant, beta_sant, yerr=beta_err_sant, ls='none', marker='.', ms=12, label='Santini+17')
    # ax2.errorbar(z_boog, beta_boog, yerr=beta_err_boog, ls='none', marker='.', ms=12, label='Boogaard+18')
    # ax2.errorbar(z_pear, beta_pear, yerr=beta_err_pear, ls='none', marker='.', ms=12, label='Pearson+18')
    # ax2.errorbar(z_donn, beta_donn, yerr=beta_err_donn, ls='none', marker='.', ms=12, label='Donnari+19')

    # ax2.plot(z_spea, beta_spea, label='Speagle+14')
    # ax2.plot(z_schr, beta_schr, label='Schreiber+15')
    # ax2.plot(z_leja, beta_leja, label='Leja+21')

    # ax2.fill_between(z_spea, beta_spea-beta_err_spea, beta_spea+beta_err_spea, alpha=0.1, color='k')

    ax2.errorbar(z_donn, beta_donn, ls='dashed', marker='.', ms=0, label='Illustris-TNG')
    ax2.errorbar(z_love, beta_love, ls='dashed', marker='.', ms=0, label='FLARES')


    ax2.errorbar(z_spea, beta_spea, ls='solid', marker='.', ms=0, label='Speagle+14')
    ax2.plot(3, 0.8)
    ax2.errorbar(z_schr, beta_schr, ls='solid', marker='.', ms=0, label='Schreiber+15')
    ax2.errorbar(z_leja, beta_leja, ls='solid', marker='.', ms=0, label='Leja+21')

    ax2.errorbar(z_stei, beta_stei, yerr=beta_err_stei, ls='none', marker='.', ms=ms_dot, label='Steinhardt+14')
    ax2.errorbar(z_whit, beta_whit, yerr=beta_err_whit, ls='none', marker='.', ms=ms_dot, label='Whitaker+14')
    ax2.errorbar(z_salm, beta_salm1, yerr=0, ls='none', marker='.', ms=ms_dot, label='Salmon+15')
    ax2.errorbar(z_kurc, beta_kurc, yerr=beta_err_kurc, ls='none', marker='.', ms=ms_dot, label='Kurczynski+16')
    ax2.errorbar(z_sant, beta_sant, yerr=beta_err_sant, ls='none', marker='.', ms=ms_dot, label='Santini+17')
    # ax2.errorbar(z_boog, beta_boog, yerr=beta_err_boog, ls='none', marker='.', ms=ms_dot, label='Boogaard+18')
    ax2.errorbar(z_pear, beta_pear, yerr=beta_err_pear, ls='none', marker='.', ms=ms_dot, label='Pearson+18')


    # ax3.errorbar(z_stei, sig_stei, ls='none', marker='x', ms=8, label='Steinhardt+14')
    # ax3.errorbar(z_sig_schr, sig_schr, ls='none', marker='x', ms=8, label='Schreiber+15')
    # ax3.errorbar(z_salm, sig_salm, ls='none', marker='x', ms=8, label='Salmon+15')
    # ax3.errorbar(z_kurc, sig_kurc, yerr=sig_err_kurc, ls='none', marker='.', ms=12, label='Kurczynski+16')
    # ax3.errorbar(z_sig_sant, sig9p2_sant, ls='none', marker='x', ms=8, label='Santini+17')
    # ax3.errorbar(z_boog, sig_boog, yerr=sig_err_boog, ls='none', marker='.', ms=12, label='Boogaard+18')
    # ax3.errorbar(z_pear, sig_pear, yerr=sig_err_pear, ls='none', marker='.', ms=12, label='Pearson+18')
    # ax3.errorbar(z_sig_leja, sig9p7_leja, ls='none', marker='x', ms=8, label='Leja+21')

    # ax3.plot(z_spea, sig_spea, label='Speagle+14')

    # ax3.fill_between(z_spea, sig_spea-sig_err_spea, sig_spea+sig_err_spea, alpha=0.1, color='k')
    ...
    ax3.errorbar(z_spea, sig_spea, ls='solid', marker='.', ms=0, label='Speagle+14')
    ax3.plot(3, 0.3)
    ax3.errorbar(z_sig_schr, sig_schr, ls='solid', marker='.', ms=0, label='Schreiber+15')
    ax3.errorbar(z_sig_leja, sig9p7_leja, ls='solid', marker='.', ms=0, label='Leja+21')

    ax3.errorbar(z_stei, sig_stei, ls='none', marker='.', ms=ms_dot, label='Steinhardt+14')
    ax3.plot(3, 0.3)
    ax3.errorbar(z_salm, sig_salm, yerr=sig_err_salm, ls='none', marker='.', ms=ms_dot, label='Salmon+15')

    ax3.errorbar(z_kurc, sig_kurc, yerr=sig_err_kurc, ls='none', marker='.', ms=ms_dot, label='Kurczynski+16')
    ax3.errorbar(z_sig_sant, sig9p2_sant, ls='none', marker='.', ms=ms_dot, label='Santini+17')
    # ax3.errorbar(z_boog, sig_boog, yerr=sig_err_boog, ls='none', marker='.', ms=ms_dot, label='Boogaard+18')
    ax3.errorbar(z_pear, sig_pear, yerr=sig_err_pear, ls='none', marker='.', ms=ms_dot, label='Pearson+18')


    ax1.legend(loc=(0.04,1.1), frameon=True, fontsize=1.4*fontsize_legend, ncol=3)
    # ax2.legend(loc="lower center", bbox_to_anchor=(0.5, -0.4), ncol=4,fontsize=12)
    ...
    for rc in [rc_c]:
        for a, ax in enumerate([ax1]):
            ax.plot(rc.item().get('z'), rc.item().get('{}_50_arr'.format(param[a])), color='red', linewidth=lw, zorder=10)
            ax.fill_between(rc.item().get('z'), rc.item().get('{}_16_arr'.format(param[a])), rc.item().get('{}_84_arr'.format(param[a])), alpha=0.3, color='red', zorder=10)
    ...
    # ax1.plot(z_225, (np.log10((1+z_225)**2.25) - 9.0) * (rc_c.item().get('ssfr_50_arr')[idx] / (np.log10((1+2.0)**2.25) - 9.0)), linestyle='--', label='$\sim\log((1+z)^{2.25})-9$') # normalise Dekel et al. 2009 to z=2 (factor of ~ 1.1)
    # ax1.plot(z_225, (np.log10((1+z_225)**2.25) - 9.0), linestyle='--', label='$\log((1+z)^{2.25})-9$') # normalise Dekel et al. 2009 to z=2 (factor of ~ 1.1)
    # ax1.plot(z_225, (np.log10((1+z_225)**2.25) - 9.0) - ((np.log10((1+2.0)**2.25) - 9.0) - rc_c.item().get('ssfr_50_arr')[idx]), linestyle='--', label='$\log$') # normalise Dekel et al. 2009 to z=2 (-0.85 = + log(0.14))
    ax1.plot(z_225, np.log10(1.4e-10*(1+z_225)**2.25), linestyle='--', label=r'$\log(\,\mathrm{N}\,(1+z)^{2.25})$') # normalise Dekel et al. 2009 to z=2 (-0.85 = + log(0.14))

    # print('TTTT',  - ((np.log10((1+2.0)**2.25) - 9.0) - rc_c.item().get('ssfr_50_arr')[idx]))

    # ax1.errorbar(z_redd, ssfr_redd, yerr=(ssfr_m_redd, ssfr_p_redd), ls='none', marker='.', ms=12, label='Reddy+12')
    # ax1.errorbar(z_barr, ssfr_barr, yerr=(ssfr_m_barr, ssfr_p_barr), ls='none', marker='.', ms=12, label='de Barros+14')
    # ax1.errorbar(z_gonz, ssfr_gonz, yerr=(ssfr_m_gonz, ssfr_p_gonz), ls='none', marker='.', ms=12, label='Gonzalez+14')
    # ax1.errorbar(z_stei, ssfr_stei, yerr=ssfr_err_stei, ls='none', marker='.', ms=12, label='Steinhardt+14')
    # ax1.errorbar(z_whit, ssfr_whit, yerr=ssfr_err_whit, ls='none', marker='.', ms=12, label='Whitaker+14')
    # ax1.errorbar(z_lee, ssfr_lee, ls='none', marker='x', ms=8, label='Lee+15')
    # ax1.errorbar(z_salm, ssfr_salm1, yerr=ssfr_err_salm1, ls='none', marker='.', ms=12, label='Salmon+15')
    # ax1.errorbar(z_kurc, ssfr_kurc, yerr=ssfr_err_kurc, ls='none', marker='.', ms=12, label='Kurczynski+16')
    # ax1.errorbar(z_sant, ssfr_sant, yerr=ssfr_err_sant, ls='none', marker='.', ms=12, label='Santini+17')
    # ax1.errorbar(z_boog, ssfr_boog, yerr=ssfr_err_boog, ls='none', marker='.', ms=12, label='Boogaard+18')
    # ax1.errorbar(z_pear, ssfr_pear, yerr=ssfr_err_pear, ls='none', marker='.', ms=12, label='Pearson+18')
    # ax1.errorbar(z_donn, ssfr_donn, yerr=ssfr_err_donn, ls='none', marker='.', ms=12, label='Donnari+19')
    # ax1.errorbar(z_thor, ssfr_thor, ls='none', marker='x', ms=8, label='Thorne+21')

    # ax1.plot(z_spea, ssfr_spea, label='Speagle+14')
    # ax1.plot(z_schr, ssfr_schr, label='Schreiber+15')
    # ax1.plot(z_tomc, ssfr_tomc, label='Tomczak+16')
    # ax1.plot(z_lesl, ssfr_lesl, label='Leslie+20')
    # ax1.plot(z_leja, ssfr_leja, label='Leja+21')

    # ax1.fill_between(z_spea, ssfr_spea-ssfr_err_spea, ssfr_spea+ssfr_err_spea, alpha=0.1, color='k')
    # ax1.fill_between(z_schr, ssfr_schr-ssfr_err_schr, ssfr_schr+ssfr_err_schr, alpha=0.1, color='k')


    ax1.errorbar(z_furl, ssfr_furl, ls='dashed', marker='.', ms=0, label='EAGLE')
    # ax1.errorbar(z_donn, ssfr_donn, yerr=ssfr_err_donn, ls='solid', marker='.', ms=0, label='Illustris-TNG')
    ax1.errorbar(z_donn, ssfr_donn, ls='dashed', marker='.', ms=0, label='Illustris-TNG')
    ax1.errorbar(z_love, ssfr_love, ls='dashed', marker='.', ms=0, label='FLARES')

    ax1.plot(3, -9.0)
    ax1.errorbar(z_spea, ssfr_spea, ls='solid', marker='.', ms=0, label='Speagle+14')
    ax1.errorbar(z_schr, ssfr_schr, ls='solid', marker='.', ms=0, label='Schreiber+15')
    ax1.errorbar(z_tomc, ssfr_tomc, ls='solid', marker='.', ms=0, label='Tomczak+16')
    ax1.errorbar(z_lesl, ssfr_lesl, ls='solid', marker='.', ms=0, label='Leslie+20')
    ax1.errorbar(z_leja, ssfr_leja, ls='solid', marker='.', ms=0, label='Leja+21')

    # ax1.errorbar(z_redd, ssfr_redd, yerr=(ssfr_m_redd, ssfr_p_redd), ls='none', marker='.', ms=ms_dot, label='Reddy+12')
    ax1.errorbar(z_star, ssfr_star, yerr=(ssfr_m_star, ssfr_p_star), ls='none', marker='.', ms=ms_dot, label='Stark+13')
    # ax1.errorbar(z_barr, ssfr_barr, yerr=(ssfr_m_barr, ssfr_p_barr), ls='none', marker='.', ms=ms_dot, label='de Barros+14')
    # ax1.errorbar(z_gonz, ssfr_gonz, yerr=(ssfr_m_gonz, ssfr_p_gonz), ls='none', marker='.', ms=ms_dot, label='Gonzalez+14')
    ax1.errorbar(z_smit, ssfr_smit, yerr=([ssfr_m_smit], [ssfr_p_smit]), ls='none', marker='.', ms=ms_dot, label='Smit+14', color='k')
    ax1.errorbar(z_stei, ssfr_stei, yerr=ssfr_err_stei, ls='none', marker='.', ms=ms_dot, label='Steinhardt+14')
    ax1.errorbar(z_whit, ssfr_whit, yerr=ssfr_err_whit, ls='none', marker='.', ms=ms_dot, label='Whitaker+14')
    ax1.errorbar(z_lee, ssfr_lee, yerr=ssfr_err_lee, ls='none', marker='.', ms=ms_dot, label='Lee+15')
    ax1.errorbar(z_salm, ssfr_salm1, yerr=ssfr_err_salm1, ls='none', marker='.', ms=ms_dot, label='Salmon+15')
    ax1.errorbar(z_kurc, ssfr_kurc, yerr=ssfr_err_kurc, ls='none', marker='.', ms=ms_dot, label='Kurczynski+16')
    ax1.errorbar(z_marm, ssfr_marm, yerr=ssfr_err_marm, ls='none', marker='.', ms=ms_dot, label='Marmol-Queralto+16')
    ax1.errorbar(z_sant, ssfr_sant, yerr=ssfr_err_sant, ls='none', marker='.', ms=ms_dot, label='Santini+17')
    # ax1.errorbar(z_boog, ssfr_boog, yerr=ssfr_err_boog, ls='none', marker='x', ms=ms_tri, label='Boogaard+18')
    ax1.errorbar(z_pear, ssfr_pear, yerr=ssfr_err_pear, ls='none', marker='.', ms=ms_dot, label='Pearson+18')
    ax1.errorbar(z_thor, ssfr_thor, yerr=ssfr_err_thor, ls='none', marker='.', ms=ms_dot, label='Thorne+21')

    # ax1.fill_between(z_spea, ssfr_spea-ssfr_err_spea, ssfr_spea+ssfr_err_spea, alpha=0.1, color='k')
    ax1.fill_between(z_schr, ssfr_schr-ssfr_err_schr, ssfr_schr+ssfr_err_schr, alpha=0.1, color='#8c564b')
    ax1.fill_between(z_lesl, ssfr_lesl-ssfr_err_lesl, ssfr_lesl+ssfr_err_lesl, alpha=0.1, color='#7f7f7f')

    ax1.legend(loc='lower right', frameon=True, fontsize=fontsize_legend, ncol=3)

    plt.text(0, -7.3, r'$\log(M \, / \, \mathrm{M_{\odot}}) \sim 9.7$')
