...
class Chain(object):
    def __init__(self, xArr, yArr, zArr, xsigArr, ysigArr, zsigArr, xycovArr, xzcovArr, yzcovArr, delta, nGaussXi, nChains, nGaussBeagle, piBeagle, proposalscale_xi, proposalscale_eta, proposalscale_zeta, proposalscale_alpha_a, proposalscale_alpha_b, proposalscale_alpha_c, proposalscale_beta_a, proposalscale_beta_b, proposalscale_beta_c, proposalscale_sig0, proposalscale_k, rng=None):
...
#            print('werrs ERROR')
        ...
        self.accept_eta = 0 # these get updated later on
        self.reject_eta = 0 # these get updated later on
        self.accept_zeta = 0 # these get updated later on
        self.reject_zeta = 0 # these get updated later on
        
        self.proposalscale_xi = proposalscale_xi
        self.proposalscale_eta = proposalscale_eta
        self.proposalscale_zeta = proposalscale_zeta
        self.proposalscale_alpha_a = proposalscale_alpha_a
        self.proposalscale_alpha_b = proposalscale_alpha_b
        self.proposalscale_alpha_c = proposalscale_alpha_c
    ...
    def initial_guess(self): # Step 1
        ...
        z_sampled = np.random.normal(z, zsig)
        fit_sample = np.polyfit(x_sampled, y_sampled, 1)
#        residual_sample = y_sampled - (fit_sample[0]*x_sampled + fit_sample[1])
        
        # starting values
#        self.alpha_a = fit_sample[1]
        self.alpha_b = 0.0 #np.random.normal(0.0, 1.0)
        self.alpha_c = 0.0 #np.random.normal(0.0, 1.0)
#        self.alpha = self.calc_alpha(zeta=self.zeta)
#        self.beta_a = fit_sample[0]
        self.beta_b = 0.0 #np.random.normal(0.0, 1.0)
        self.beta_c = 0.0 #np.random.normal(0.0, 1.0)
#        self.beta = self.calc_beta(zeta=self.zeta)
#        self.sig0 = np.std(residual_sample)
#        self.k = max(0.0, np.random.normal(1.0, 0.2))
#        print(self.alpha_a, self.alpha[0], self.beta_a, self.beta[0], self.sig0, self.k)

        # fixed starting values (to match mock at least)
#        self.alpha_a = -9.0
#        self.alpha_b = 0.5
#        self.alpha = self.calc_alpha(zeta=self.zeta)
#        self.beta_a = 1.0
...
#        self.k = 1.0
        ...
        # making better estimates for starting points based upon redshift bins
        # assuming linear alpha, and constant beta
        width = (max(z_sampled)-min(z_sampled)) / 10.0
        z_midpoints = np.linspace(min(z_sampled)+width, max(z_sampled)-width, 5)
        print(z_midpoints)
        alpha_samples = []
        beta_samples = []
        for z_midpoint in z_midpoints:
            idx_z = (abs(z_sampled - z_midpoint) < width)
            fit_sample = np.polyfit(x_sampled[idx_z], y_sampled[idx_z], 1)
            alpha_samples.append(fit_sample[1])
            beta_samples.append(fit_sample[0])
                
        self.alpha_a = np.polyfit(z_midpoints, alpha_samples, 1)[1]
        self.alpha_b = np.polyfit(z_midpoints, alpha_samples, 1)[0]
        self.beta_a = np.mean(beta_samples)

        # list of sfrs according to input values 
        y_from_relation = (self.alpha_a + (self.alpha_b * z_sampled)) + (np.mean(beta_samples) * x_sampled)
        y_residuals = y_sampled - y_from_relation
        
        self.alpha = self.calc_alpha(zeta=self.zeta)
        self.beta = self.calc_beta(zeta=self.zeta)
        self.sig0 = np.std(y_residuals)
        self.k = 1.0
        print('INITIAL GUESSES', self.alpha_a, self.alpha_b, self.beta_a, self.sig0, self.k)
        print('INITIAL GUESSES', self.alpha_a, self.alpha_b, self.beta_a, self.sig0, self.k, min(z_sampled), max(z_sampled), width)        
...
#                P(xi|eta,zeta,x,y)
    ...
    def update_eta(self):  # Step 4 - MH required
        ...
        for i in range(len(self.xi)):
            if wyerr[i] == True: # always: boolean array where ysig != 0
                ...
                if u <= acceptanceProb:
                    ...
                    if i == 0:
                        self.accept_eta = self.accept_eta + 1
                else:
                    if i == 0:
                        self.reject_eta = self.reject_eta + 1

                test = self.accept_eta+self.reject_eta
                if self.ichain >= self.nBurn and test > 0 and test % self.testIter == 0:
                    if np.float(self.accept_eta)/np.float(self.accept_eta+self.reject_eta) > 0.5:
                        self.proposalscale_eta = self.proposalscale_eta*1.1
                    elif np.float(self.accept_eta)/np.float(self.accept_eta+self.reject_eta) < 0.4:
                        self.proposalscale_eta = self.proposalscale_eta*0.9
    ...
    '''
    def update_eta(self):  # Step 4 - not MH
#        print('update_eta')
        wxerr = self.wxerr
        wyerr = self.wyerr
        ...
        for i in range(len(self.xi)):
            if wzerr[i] == True: # always: boolean array where ysig != 0
                ...
                if u <= acceptanceProb:
                    ...
                    if i == 0:
                        self.accept_zeta = self.accept_zeta + 1
                else:
                    if i == 0:
                        self.reject_zeta = self.reject_zeta + 1

                test = self.accept_zeta+self.reject_zeta
                if self.ichain >= self.nBurn and test > 0 and test % self.testIter == 0:
                    if np.float(self.accept_zeta)/np.float(self.accept_zeta+self.reject_zeta) > 0.5:
                        self.proposalscale_zeta = self.proposalscale_zeta*1.1
                    elif np.float(self.accept_zeta)/np.float(self.accept_zeta+self.reject_zeta) < 0.4:
                        self.proposalscale_zeta = self.proposalscale_zeta*0.9
...
#        print('update_alpha_beta_sigma')
        ...
        if self.ichain <= self.nBurn:
            ...
            sig0_prop,k_prop = self.rng.normal(loc=[alpha_a_curr,alpha_b_curr,alpha_c_curr, \
            ...
            alpha_a_prop,alpha_b_prop,alpha_c_prop, \
            beta_a_prop,beta_b_prop,beta_c_prop, \
            sig0_prop,k_prop = self.rng.multivariate_normal([alpha_a_curr,alpha_b_curr,alpha_c_curr, \
                                              beta_a_curr,beta_b_curr,beta_c_curr,    \
                                              sig0_curr,k_curr], covProposal)

#            print('FIRST',alpha_a_prop,alpha_b_prop,alpha_c_prop, \
#            beta_a_prop,beta_b_prop,beta_c_prop, \
#            sig0_prop,k_prop)
            
#            covProposal = np.cov(np.array((self.chain['alpha_a'][self.ichain-nBurn:self.ichain], \
#                                           self.chain['alpha_b'][self.ichain-nBurn:self.ichain], \
#                                           self.chain['alpha_c'][self.ichain-nBurn:self.ichain], \
#                                           self.chain['beta_a'][self.ichain-nBurn:self.ichain],  \
#                                           self.chain['beta_b'][self.ichain-nBurn:self.ichain],  \
#                                           self.chain['beta_c'][self.ichain-nBurn:self.ichain],  \
#                                           self.chain['sig0'][self.ichain-nBurn:self.ichain],   \
#                                           self.chain['k'][self.ichain-nBurn:self.ichain])))
#            
#            alpha_a_prop,alpha_b_prop,alpha_c_prop, \
#            beta_a_prop,beta_b_prop,beta_c_prop, \
#            sig0_prop,k_prop = self.rng.multivariate_normal([alpha_a_curr,alpha_b_curr,alpha_c_curr, \
...
#            print('SECOND',alpha_a_prop,alpha_b_prop,alpha_c_prop, \
#            beta_a_prop,beta_b_prop,beta_c_prop, \
#            sig0_prop,k_prop)
        ...
        prior_curr = self.prior_all(alpha_a_curr, alpha_b_curr, alpha_c_curr, beta_a_curr, beta_b_curr, beta_c_curr, sig0_curr, k_curr)
        prior_prop = self.prior_all(alpha_a_prop, alpha_b_prop, alpha_c_prop, beta_a_prop, beta_b_prop, beta_c_prop, sig0_prop, k_prop)
...
#        print('dunif')
    ...
    def prior_alpha(self,alpha_a,alpha_b,alpha_c):
#        print('prior_alpha')
        min_alpha_a = -1e6
        min_alpha_b = -1e6
        min_alpha_c = -1e6
        ...
        max_alpha_b = 1e6
        max_alpha_c = 1e6
        a = self.dunif(alpha_a, min_alpha_a, max_alpha_a)
        b = self.dunif(alpha_b, min_alpha_b, max_alpha_b)
        c = self.dunif(alpha_c, min_alpha_c, max_alpha_c)
        return a*b*c
...
#        print('prior_k')
    ...
    def prior_all(self, alpha_a, alpha_b, alpha_c, beta_a, beta_b, beta_c, sig0, k):
#        print('prior_all')
        return self.prior_alpha(alpha_a,alpha_b,alpha_c)*self.prior_beta(beta_a,beta_b,beta_c)*self.prior_sig0(sig0)*self.prior_k(k)
...
#        print('calc_sigsqr')
    ...
    def calc_alpha(self, zeta=[0], alpha_a=None, alpha_b=None, alpha_c=None):
        ...
        alpha = alpha_a + alpha_b*np.array(zeta)#+ alpha_b*np.array(zeta) # + alpha_b * (1/np.array(zeta)) + alpha_c * ((1/np.array(zeta))**2)
#        alpha = alpha_a + 0*np.array(zeta) # + alpha_b * (1/np.array(zeta)) + alpha_c * ((1/np.array(zeta))**2)
        return alpha
    ...
    def calc_beta(self, zeta=[0], beta_a=None, beta_b=None, beta_c=None):
        ...
        beta = beta_a + 0*np.array(zeta)#+ beta_b*np.array(zeta) # + beta_b * (1/zeta) + beta_c * ((1/zeta)**2)
#        beta = beta_a + 0*np.array(zeta) # + beta_b * (1/zeta) + beta_c * ((1/zeta)**2)
        return beta    
...
class LinMix(object):
    ...
    """
    def __init__(self, xArr, yArr, zArr, xsigArr, ysigArr, zsigArr, xycovArr, xzcovArr, yzcovArr, proposalscale_xi, proposalscale_eta, proposalscale_zeta, proposalscale_alpha_a, proposalscale_alpha_b, proposalscale_alpha_c, proposalscale_beta_a, proposalscale_beta_b, proposalscale_beta_c, proposalscale_sig0, proposalscale_k, delta=None, nGaussXi=3, nChains=4, parallelize=True, seed=None, nGaussBeagle=1, piBeagle=None):
        ...
        if self.parallelize:
            ...
            init_kwargs0 = {'xArr':xArr,
                            ...
                            'proposalscale_alpha_a':proposalscale_alpha_a,
                            'proposalscale_alpha_b':proposalscale_alpha_b,
                            'proposalscale_alpha_c':proposalscale_alpha_c,
    ...
    def _get_psi(self):
        ...
        psi={}
        keys=['alpha_a','alpha_b','alpha_c','beta_a','beta_b','beta_c','sig0','k']
        for key in keys:
        ...
        keys=['alpha_a','alpha_b','alpha_c','beta_a','beta_b','beta_c','sig0','k','ximean','xivar']
        for key in keys:
    ...
    def run_mcmc(self, minIter=5000, maxIter=100000, silent=False, nBurn=500, checkIter=5000, RhatLimit=5000, testIter=200):
        ...
        for i in range(0, minIter, checkIter): # 0 to minIter, whilst checking for convergence every "checkIter"
            ...
            print('Rhat values:')
            print('alpha:', Rhat['alpha_a'], Rhat['alpha_b'], Rhat['alpha_c'])
...
#data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/scenario_7_7_data_z1p3.p','r')) # per bin
#data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/mock_007.p','r')) # creating_a_mock.py - gaussian masses and redshifts
#data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/mock_009.p','r')) # creating_a_mock.py - real masses and redshifts
#data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/mock_110.p','r')) # creating_a_mock.py - 101 - 110 are bias tests
...
nChains                 = 4
minIter                 = 2000                  # if i%(niter/10.0) == 0:, TENTH OF THIS NEEDS TO DIVIDE BY 4, see yellow below
maxIter                 = minIter
checkIter               = minIter           # must be divisible by 4
nBurn                   = minIter/10        # this is when the MH proposal distributions start adapting (xi, eta, zeta, alpha, beta, sig0 and k)
testIter                = minIter/20                   # xi, eta, zeta MH proposal checks every "testIter" iterations and adjusts proposal if needed
RhatLimit               = 1.01

#nBurn                   = minIter        # this is when the MH proposal distributions start adapting (xi, eta, zeta, alpha, beta, sig0 and k)
#testIter                = minIter        # xi, eta, zeta MH proposal checks every "testIter" iterations and adjusts proposal if needed


#proposalscale_xi        = 0.25
#proposalscale_eta       = 0.25
#proposalscale_zeta      = 0.25
#proposalscale_alpha_a   = 0.1                   # usually 0.1
#proposalscale_alpha_b   = 0.0                   # usually 0.05
#proposalscale_alpha_c   = 0.0                   # usually 0.0
#proposalscale_beta_a    = 0.01                  # usually 0.01
#proposalscale_beta_b    = 0.0                  # usually 0.01
#proposalscale_beta_c    = 0.0                  # usually 0.0
#proposalscale_sig0      = 0.05                  # usually 0.05
#proposalscale_k         = 0.00                  # usually 0.01
#
#parallelize             = True
...
#proposalscale_alpha_a   = 0.1                   # usually 0.005
#proposalscale_alpha_b   = 0.005                   # usually 0.005
proposalscale_alpha_c   = 0.0                   # usually 0.005
#proposalscale_beta_a    = 0.01                  # usually 0.005
proposalscale_beta_b    = 0.0                  # usually 0.005
proposalscale_beta_c    = 0.0                  # usually 0.005
#proposalscale_sig0      = 0.003                  # usually 0.005
proposalscale_k         = 0.00                  # usually 0.01


proposalscale_alpha_a   = 0.02                   # usually 0.005
proposalscale_alpha_b   = 0.005                  # usually 0.005
proposalscale_beta_a    = 0.001                # usually 0.005
proposalscale_sig0      = 0.001                  # usually 0.005
...
lm = LinMix(GMMx, GMMy, GMMz, GMMxsig, GMMysig, GMMzsig, xycovArr=GMMxycov, xzcovArr=GMMxzcov, yzcovArr=GMMyzcov, nGaussXi=nGaussXi, nGaussBeagle=nGaussBeagle, piBeagle=piBeagle, nChains=nChains, parallelize=parallelize, proposalscale_xi=proposalscale_xi, proposalscale_eta=proposalscale_eta, proposalscale_zeta=proposalscale_zeta, proposalscale_alpha_a=proposalscale_alpha_a, proposalscale_alpha_b=proposalscale_alpha_b, proposalscale_alpha_c=proposalscale_alpha_c, proposalscale_beta_a=proposalscale_beta_a, proposalscale_beta_b=proposalscale_beta_b, proposalscale_beta_c=proposalscale_beta_c, proposalscale_sig0=proposalscale_sig0, proposalscale_k=proposalscale_k)

lm.run_mcmc(minIter=minIter, maxIter=maxIter, nBurn=nBurn, checkIter=checkIter, testIter=testIter, RhatLimit=RhatLimit)
    
#pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_mock_006_fit_018.p','w'))
#pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_redshift_test_032.p','w'))

#pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_bias_test_110.p','w'))
