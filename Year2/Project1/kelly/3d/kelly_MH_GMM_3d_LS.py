...
#        self.beta = fit_sample[0]
        ...
        print(self.alpha_a, self.alpha[0], self.beta_a, self.beta[0], self.sig_0, self.k)
...
#        prior_prop = 1
        ...
        prior_curr = self.prior_all(alpha_a_curr, alpha_b_curr, alpha_c_curr, beta_a_curr, beta_b_curr, beta_c_curr, sig_0_curr, k_curr)
        prior_prop = self.prior_all(alpha_a_prop, alpha_b_prop, alpha_c_prop, beta_a_prop, beta_b_prop, beta_c_prop, sig_0_prop, k_prop)
...
#        print('prior_k')
    ...
    def prior_all(self, alpha_a, alpha_b, alpha_c, beta_a, beta_b, beta_c, sig_0, k):
#        print('prior_all')
        return self.prior_alpha(alpha_a,alpha_b,alpha_c)*self.prior_beta(beta_a,beta_b,beta_c)*self.prior_sig_0(sig_0)*self.prior_k(k)
...
nChains     = 4
minIter     = 100 # if i%(niter/10.0) == 0:, TENTH OF THIS NEEDS TO DIVIDE BY 4, see yellow below
maxIter     = 5*minIter
n_burn      = 0.1*minIter # this is when the proposal distribution starts learning from the previous points

'''
NOTE checkiter is set roughly line 1078
currently set to checkiter = miniter/10, which for rhat is halved, and havled again
'''
