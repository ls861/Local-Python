...
option_mock                         = 1 # 1 is usual mock, 2 inc GMMz, GMMxsig, GMMxzcov, GMMyzcov, 0 is AD (old and new)
...
# never made mass or sfr inputs for mock, hence NA for MS options
'''
option_mock                         = 0 # 1 is usual mock, 2 inc GMMz, GMMxsig, GMMxzcov, GMMyzcov, 0 is AD (old and new)
...
option_ind_objects_mass             = 1
option_ind_objects_sfr              = 1
option_ind_objects_z                = 1 # 2 only, redshift dependence hack
option_samples_from_fitted_model    = 1
...
option_chains_2x2                   = 0
option_chains_4x1                   = 0
option_ind_objects_mass             = 0
option_ind_objects_sfr              = 0
option_ind_objects_z                = 0 # 2 only, redshift dependence hack
...
# MOCK
comment2    = '001'        # 001 is k==1, 002 is fitted k
nChains     = 4             # options are 1 or 4
minIter     = 50000            # or 5000
burn        = 30000

## REAL POSTER
#comment2    = '001'        # 1, 2 or 3, not sure what the differences are... (ALL IDENTICAL)
#nChains     = 4             # options are 1 or 4
...
#    true_values = {'alpha':-6.2, 'beta':0.8, 'sig_0':0.3, 'k':1.0}
    true_values = {'alpha':-6.1548, 'beta':0.756, 'sig_0':0.3, 'k':1.0}    
...
if option_mock == 2:
    ...
    print('CHAIN', sbf+'lm_chain_{}x_{}_{}.npy'.format(str(nChains), str(minIter), comment2))
...
if option_chains == 1:
    ...
    #    plt.title('Histogram representing the posterior distribution of {}'.format(param.replace('_', '')))
        ...
        if option_mock == 1:
            plt.plot((true_values[param], true_values[param]),(0, y.max()), color='k', label='Input')
        plt.legend()
#        plt.tight_layout()
#        plt.savefig('/Users/lester/Dropbox/PhD/20_Summer/First Year Report/RawFigs/320_{}_mock.png'.format(param)) 
        plt.show()
...
if option_chains_4x1 == 1:
    ...
    #    axs[k].set_xlabel(param.replace('_', ''))
        ...
        if option_mock == 1:
            axs[k].plot((true_values[param], true_values[param]),(0, y.max()), color='k', label='Input')
        axs[k].legend()
...
#    plt.tight_layout()
#    plt.savefig('/Users/lester/Dropbox/PhD/20_Summer/First Year Report/RawFigs/320_4x1_mock.png') 
    plt.show()
