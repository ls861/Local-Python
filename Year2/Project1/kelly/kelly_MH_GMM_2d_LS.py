...
class Chain(object):
    def __init__(self, xArr, yArr, xsigArr, ysigArr, xycovArr, delta, K, nchains, nGMM_err, pi_err, rng=None):
        ...
        self.proposalscale_alpha = 0.5 # usually 0.005
        self.proposalscale_beta = 0.05 # usually 0.005
        self.proposalscale_sig_0 = 0.05 # usually 0.005
...
### 8p0 2p0 0p5 9p5 2d 005 eta gibbs
### 8p0 2p0 0p5 9p5 2d 006 eta MH
### 8p5 1p65 0p35 9p5 2d 001
### 8p5 1p65 0p35 2p5 2d 001
### 8p5 (1p65, 2p5, 3p5, 4p5) (2p5, 9p5) 2d 001
...
comment2    = '002' # 003 onwards is fixed sig and k
nChains     = 4
minIter     = 500 # if i%(niter/10.0) == 0:
maxIter     = minIter
nburn       = 100

lm = LinMix(GMMx, GMMy, GMMxsig, GMMysig, xycovArr=GMMxycov, K=nK, nGMM_err=nGauss, pi_err=pi_err, nchains=nChains)
lm.run_mcmc(miniter=minIter, maxiter=maxIter, n_burn=nburn)

# trying to fix rhat and convergence etc
np.save('/Users/lester/Documents/linmix_files/lm_chain_mass{}_z{}_w{}_chi{}_dim{}{}{}_com{}_{}x_{}_com{}'.format(massLim, zLim, wLim, chi2Lim, dimension, massType, sfrType, comment, str(nChains), str(minIter), comment2), lm.chain)
...
##options = 'z5p0'
#
#data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/data_{}.p'.format(options),'r'))
#
#pi_err      = data['amp_GMM']           # 3x probability of each posterior gaussian
...
#
#nK          = 3                         # 3 #gaussians modelling xi
#nGauss      = 3                         # 3 #gaussians modelling BEAGLE posterior
#
#nChains     = 4
#minIter     = 5000 # if i%(niter/10.0) == 0:
#maxIter     = minIter
#nburn       = 1000 # this is when the proposal distribution starts learning from the previous points
#
#lm = LinMix(GMMx, GMMy, GMMxsig, GMMysig, xycovArr=GMMxycov, K=nK, nGMM_err=nGauss, pi_err=pi_err, nchains=nChains)
#lm.run_mcmc(miniter=minIter, maxiter=maxIter, n_burn=nburn)
#
...
#pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_new_{}_test.p'.format(options),'w'))
...
#    data = pickle.load(open('/Users/lester/Documents/GitHub/Local-Python/Year2/Project1/kelly/data/scenario_5_7_data_{}.p'.format(options[i]),'r'))
    ...
    nChains     = 4
    minIter     = 20 # if i%(niter/10.0) == 0:
    maxIter     = minIter
    nburn       = 2 # this is when the proposal distribution starts learning from the previous points
...
#    pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_scenario_6_7_data_{}_k0.p'.format(options[i]),'w'))
#    pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_scenario_6_7_data_{}_k0_002.p'.format(options[i]),'w'))
#    pickle.dump(lm.chain, open('/Users/lester/Documents/linmix_files/lm_chain_scenario_6_7_data_{}_k_002.p'.format(options[i]),'w'))
