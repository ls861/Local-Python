...
fileName = '/Users/lester/BEAGLE/BEAGLE-general/results/Lester_z_shift_grid_neb_7/my_first_test.fits'
...
for i in range(len(data_fits[7].data)):
    ...
    ### Apparently simplified    
    #data_photo = (10**10) * (c / np.array(filter_fwhm_centre)) * (10 ** ((np.array(data_photo_AB[i])+48.6)/-2.5))
    #print(data_photo)
    ...
    plt.scatter(filter_fwhm_centre, data_photo, marker = 'x', color = 'k', s = 100, zorder = 4)
    plt.errorbar(filter_fwhm_centre, data_photo, xerr=np.array(filter_fwhm)/2, linestyle='None', color = colour[i], zorder = 3)



### Calculated Photometric Points ###
...
### comparing f_lambda, with lambda f_lambda ###
'''
plt.figure(figsize=(16,12)) 
plt.xlabel('Wavelength $\lambda$ ($\AA$)', fontsize=14)
...
ax1.set_xlim(3000,55000)
#ax1.set_xlim(13000,13040)
#ax1.set_ylim(10**-20, 10**-14)

ax1.set_yscale('log')
ax1.set_ylabel('Energy Density $F_\lambda$ ($erg\:s^{-1}\:cm^{-2}\:\AA^{-1}$)', fontsize=14)
...
for i in range(len(data_fits[7].data)):
    ...
    plt.plot(wl_spec_z, wl_spec[::sl] * data_spec[::sl] / wl_spec_z, label = 'z = %i' % redshift[i][0], zorder = 2)
