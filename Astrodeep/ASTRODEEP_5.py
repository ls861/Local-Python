...
test = cat[cat['ZBEST']<0]

print(test['field'], test['ID'], test['ZBEST'])
print(len(test))

print('len',len(cat['RELFLAG'][cat['ZBEST']<0])) #45
print('sum',sum(cat['RELFLAG'][cat['ZBEST']<0])) #0

cat       = cat[cat['ZBEST'] > 0]


print(np.array_equal(cat['ID'], cat['b_ID']))
...
for i in range(len(tcp)):
    plt.hist(tcp[i]['ZBEST'], bins=np.linspace(0, 10.00000001, 11), histtype='step', label=title[i]) # this puts 10.0 max value in bin 9-10.

plt.title('Count of galaxies plotted against Redshift')
plt.xlabel('Redshift')
plt.ylabel('Count')
plt.legend()
plt.show()
...
z_bins = 10
filter_to_plot = ['CH1', 'H160']



for k in range(len(filter_to_plot)):
    for i in range(len(tcp)):
        
        for j in range(z_bins): # redshifts

            # , bins=np.linspace(15, 33, 40)  
            plt.hist(tcp[i][filter_to_plot[k]][ (tcp[i]['ZBEST'] > j) & (tcp[i]['ZBEST'] <= j+1)], bins=np.arange(0, 50, 0.5), label='%i $<$ z $\leq$ %i' % (j, j+1)) # this strips all -ve mags, MUST amend
            
        plt.title(title[i] + ' ' + filter_to_plot[k])
        plt.xlim(15, 31)
        plt.xlabel('Apparent Magnitude ' + filter_to_plot[k])
        plt.ylabel('Count')
        plt.legend()
        plt.show()
...
uv = (1+cat['ZBEST']) * 1500        # UV rest frame per galaxy
la = (1+cat['ZBEST']) * 1216        # lyman alpha rest frame per galaxy
...
np.save('filter_uv', filter_firstguess)

# plot quantity of filters
plt.hist(filter_firstguess, bins=np.arange(0, 11, 1))

plt.title('Count of best match filter corresponding to rest frame 1500A')
plt.xlabel('Filter')
plt.ylabel('Count')
plt.show()


# plot distribution of 'deredshifted' central filter wavelengths
plt.hist(filter_fwhm_centre[filter_firstguess] / (1+cat['ZBEST']), bins=30) # amazed that this worked as expected, gives array of chosen central wavelengths converted to rest frame

plt.title('Count plotted against the deredshifted filter central wavelength')
plt.xlabel('Deredshifted filter central wavelength')
plt.ylabel('Count')
plt.show()
...
filter_chosen_a = filter_label[filter_firstguess] # creates array of filter names 
...
plt.ylabel('Count')
plt.xscale('log')
plt.hist(uv_flux, bins=logbins)
plt.show()


# calculating absolute mags

m_app = -2.5 * np.log10(  (uv_flux * (10**-6))  /  3631) # calculated apparent mags from flux

# m_app_from_cat is apparent magnitudes from catalog for a comparison with my calculated ones frm the flux

m_app_from_cat = np.zeros(len(cat))

for i in range(len(cat)):
    m_app_from_cat[i] = cat[filter_chosen_a[i]][i]

'''
print(m_app)
print(m_app_from_cat)
print(len(m_app))
print(len(m_app_from_cat))
...
tcp = [cat, cat[np.mod(cat['field'], 2) == 0], cat[np.mod(cat['field'], 2) == 1]]
tcp_m_abs = [m_abs, m_abs[np.mod(cat['field'], 2) == 0], m_abs[np.mod(cat['field'], 2) == 1]]


for i in range(len(tcp_m_abs)):
    
    for j in range(z_bins):
        
        plt.hist(tcp_m_abs[i][ (tcp[i]['ZBEST'] > j) & (tcp[i]['ZBEST'] <= j+1)], bins=np.arange(-35, 5, 0.2), label='%i $<$ z $\leq$ %i' % (j, j+1)) # this strips all -ve mags, MUST amend
...
print(cat['H160'][cat['H160'] < 0])
print(cat['RELFLAG'][cat['H160'] < 0])
print(cat['RELFLAG'][cat['b_H160'] < 0])

print(cat['b_H160'][cat['ZBEST'] == 99])
