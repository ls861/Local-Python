...
tau         = np.random.uniform(low=8,      high=10,    size=size)          # log, width of tophat
...
for i in range(size):
    ...
    if t[i] - t0[i] < t[i] - age_z_15:
        ...
        if t_yr - t0_yr < 0:
            ...
            A = (-m*(tau_yr**2)) / (((np.exp(-(t_yr-t0_yr)/tau_yr))*((t0_yr**2)-(2*t0_yr*(tau_yr+t_yr))+(2*(tau_yr**2))+(2*tau_yr*t_yr)+(t_yr**2)))-(2*(tau_yr**2)))

        sfr1 = A * np.heaviside(t_yr-t0_yr, 1) * ( ((t_yr-t0_yr)**2)/(tau_yr**3) ) * np.exp(-(t_yr-t0_yr)/tau_yr)
